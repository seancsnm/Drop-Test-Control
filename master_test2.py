...
def ReadyTest():
  ...
  print('dock=%d, grip=%d, top=%d, btm=%d, gate=%d, mtr_sw=%d' %(_dock_sensor, _grip_rel_sensor, _top_vel_sensor, _btm_vel_sensor, _gate_state, _usr_ctrl_sw))
  if (_grip_rel_sensor and _top_vel_sensor and _btm_vel_sensor): #Proceed with setting up
    ...
    M3.Refresh()
    print('Timer Active State = %d' %M3.GetLBit(D3.STATUS,D3.TIMER_ACTIVE))
    print('Test Ready State = %d' %M3.GetLBit(D3.STATUS,D3.TEST_READY))
