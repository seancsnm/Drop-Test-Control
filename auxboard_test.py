...
##Read/Write Tests
#
#instrument.write_register (CUR_CTRL, 20, 0, functioncode=6)
instrument.write_register(CTRL,0b0001, 0, functioncode=6)
#instrument.write_register(GRIP_RELEASE_TIME, 2000, functioncode=6);
for i in range(1):
  print('\n---------- i = %d -----------' %(i))
  success = 0
  minimalmodbus._print_out('\STATUS_0x00: '                        + bin(instrument.read_register(STATUS)))
  minimalmodbus._print_out('\CTRL_0x01: '                          + bin(instrument.read_register(CTRL)))
  minimalmodbus._print_out('\GRIP_RELEASE_TIME_0x02: '             + str(instrument.read_register(GRIP_RELEASE_TIME)))
  minimalmodbus._print_out('\TIMER_0x03: '                         + str(instrument.read_register(TIMER)))
  minimalmodbus._print_out('\COUNTER_0x04: '                       + str(instrument.read_register(COUNTER)))
  minimalmodbus._print_out('\DEBUG_0x05: '                         + bin(instrument.read_register(DEBUG)))

 #instrument.write_register(CTRL, 
